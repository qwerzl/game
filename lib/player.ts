...
export class Player {
  ...
  playAnimation(name: string, loop = true) {
    ...
    if (nextClip && this.animationMixer) {
      ...
      const nextAction = this.animationMixer.clipAction(nextClip).setLoop(loopMode, Infinity)
  ...
  move(clockDelta: number) {
    ...
    if (this.timeOfDeath && this.world.clock!.elapsedTime! - this.timeOfDeath > 2.5) {
      this.dead = true
    }
    ...
    const trueBearingZ = t * this.speed * Math.sin(this.bearing + Math.PI / 2)
    const perpBearingX = t * this.speed * Math.cos(this.bearing)
    const perpBearingZ = t * this.speed * Math.sin(this.bearing)
