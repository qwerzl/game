import {
  AmbientLight, type AnimationMixer, CineonToneMapping,
  Clock,
  Color,
  DefaultLoadingManager, DirectionalLight, type Group,
  HemisphereLight,
  type LoadingManager, MeshBasicMaterial, MeshStandardMaterial, type Object3D, PCFSoftShadowMap, type PerspectiveCamera,
  Scene,
  WebGLRenderer
} from "three";
...
export class World {
  ...
  loadFbx(name: string, filename: string, addToScene = false, cb = (error: Error | null, result: any) => {}) {
    ...
    fbxLoader.load(filename, (object) => {
      ...
      if (addToScene && this.scene) this.scene.add(object);
      cb(null, object);
    }, (xhr: any) => {
  ...
  addEffect(pass: Pass, camera: PerspectiveCamera) {
    if (this.effectComposer && this.renderer) this.effectComposer.addPass(new pass(
      {
