...
export class World {
  ...
  loadFbx(name: string, filename: string, addToScene = false, cb = () => {}) {
    const fbxLoader = new FBXLoader(this.loader)
    fbxLoader.load(filename, (object: any) => {
      ...
      if (this.loadedFbx[name]) {
        console.warn(`Warning: overwriting existing FBX '${name}'!`)
      }
  ...
  addEffect(RenderPass: Pass, camera: PerspectiveCamera) {
    if (this.effectComposer && this.renderer) {
      this.effectComposer.addPass(new RenderPass(
        {
